...
const CardDetailsPage = () => {
  ...
  const { data: reactions, refetch } = useQuery({
    ...
    queryFn: () =>
      fetch(`${process.env.REACT_APP_API_URL}reactions/${params?.id}`).then(
        (res) => res.json()
      ),
  });
    ...
    <div>
      {searchContent ? (
        ...
        <div className="flex w-full gap-0 lg:gap-5 h-full">
          ...
          <div>
            <h1 className="text-center text-xl font-bold text-red-500 pb-5 hidden lg:block">
              LATEST NEWS
            </h1>
            <div className="lg:w-[500px]  hidden lg:flex gap-3  flex-col">
              ...
              {recentData &&
                recentData.map((recent) => (
                  <LatestNews recent={recent} key={recent._id} />
                ))}
            </div>
            <div className="mt-10 hidden lg:block">
              <img
                className="object-cover w-[500px] h-[700px]"
                src="https://cdn.ecommercedns.uk/files/8/242988/9/13322649/christmas-banner-1.png"
                alt=""
              />
            </div>
          </div>
