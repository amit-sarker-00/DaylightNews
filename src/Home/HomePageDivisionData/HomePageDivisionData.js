        ...
        <div className='flex  flex-col-reverse xl:flex-row' >
            <div className='w-full xl:w-[1400px]'>
                ...
                <div className='flex flex-col lg:flex-row '>
                    <div>
                        {
                            datas && datas?.slice(1, 2).map(data => <HomePageDivisionSingleCard data={data} key={data?._id} />)
                        }
                    </div>
                    <div className='p-5 '>
                        {
                            datas && datas?.map(data => <DivisionTitleData data={data} key={data?._id} />)
                        }
                    </div>
                </div>
                <div className=' gap-5 pb-5 grid grid-flow-cols-1 md:grid-cols-2 sm-flex xl:grid-cols-4 lg:grid-cols-3'>
                    {
                        datas && datas?.slice(1, 5).map(data => <DivisionHorizontalData data={data} key={data?._id} />)
                    }
                </div>
            </div>
            <div className='w-full sm:w-[800px]  mx-auto xl:w-[300px] '>
                ...
                <div className='p-2 '>
                    <img className='rounded-lg  w-full sm:w-[800px]  mx-auto xl:w-[300px] xl:h-[450px]' src="https://d1csarkz8obe9u.cloudfront.net/posterpreviews/mtn-data-bundle-advert-design-template-086b970e7957e55a44ea4f181feb5a4b_screen.jpg?ts=1637022062" alt="" />
                </div>
            </div>
        </div>
    );
