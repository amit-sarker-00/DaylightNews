...
const LiveCardanoCryptoPlatprom = () => {
    ...
    useEffect(() => {
        ...
        fetch(`https://www.alphavantage.co/query?function=DIGITAL_CURRENCY_DAILY&symbol=${symbol?.ADA}&outputsize=compact&market=CNY&apikey=${API_KEY}`)
            .then(res => res.json())
            .then(data => {
             ...
             mx-auto flex items-center gap-5 mt-20 justify-center'>
                {Object.entries(allValue).slice(0, 1).map(([key, value]) => (
                    <>
                        ...
                        <p className='text-2xl font-bold'>Cardano
                            <span className='font-normal text-sm'>{symbol?.ADA}</span></p>
...
export default LiveCardanoCryptoPlatprom;